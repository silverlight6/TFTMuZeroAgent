...
class champion:
    ...
    def attack(self, bonus_dmg = 0, target = None, item_attack = False, trait_attack = '', set_AD = None):
        ...
        if(target or self.target):
            if((not self.target or self.target.health <= 0) and len(attackable_enemies) > 0): field.find_target(self)
    ...
    def spell(self, target, dmg, true_dmg = 0, item_damage = False, burn_damage = False, trait_damage = False):
        ...
        if(self == target): enemy_team = self.team #when ionic sparking themselves
        ...
        if(not ('trap_claw' in target.items and not item_damage and not burn_damage)): #trap_claw
            ...
            damage = 0
            if(not item_damage): dmg *= items.giant_slayer(self, target) #giants_slayer
            if(target.MR >= 0): damage = dmg * (100/(100+target.MR)) * self.SP
            else: damage = dmg * (2- 100/(100 - target.MR)) * self.SP

            #SP doesnt affect items' damage
            if(item_damage): damage /= self.SP
            ...
            if(target.immune): damage = 0

            crit_random  = random.randint(1,100)/100
            crit_string = ''
                #jeweled gauntlet -item                                                                       #bramble vest -item
            if('jeweled_gauntlet' in self.items and crit_random < self.crit_chance and self != target and not 'bramble_vest' in target.items and not item_damage):
            ...
            if(crit_random < self.crit_chance and self != target and not 'bramble_vest' in target.items and not item_damage and crit_string == '' and origin_class.is_trait(self, 'assassin') and origin_class.get_origin_class_tier(self.team, 'assassin') > 0):
                damage *= self.crit_damage
                crit_string = 'crit'
                

            burn_string = ''
            if(burn_damage): burn_string = ' burning'

            item_string = ''
            if(item_damage): item_string = ' item'

            trait_string = ''
            if(trait_damage): trait_string = ' {}'.format(trait_damage) 
            ...
            if(target in eval(enemy_team)):
                ...
                self.print(' deals ' + '{:<8}'.format(enemy_team) + ' ' + '{:<13}'.format(target.name) + '{:<5}--> {:<8}   shield {:<5}--> {:<5} {}{}{}{}'.format(ceil(target.health), ceil(target.health - damage), ceil(shield_old), ceil(target.shield_amount()), crit_string, burn_string, item_string, trait_string))
                ...
                if(target.health <= 0):
                        target.die()
                else: origin_class.divine(self, target, False) #divine -trait

                #sharpshooter -trait
                if(not trait_damage and not item_damage and not burn_damage): origin_class.sharpshooter(self, target, dmg, true_dmg, True)
    ...
    def move(self, y, x, forced = False, sett = False):
        if(self.idle or forced):
            ...
            else: self.print(' moves from ({} , {})   to   ({} , {})        '.format(self.y, self.x, y, x)) 
    ...
    def ability(self):
        attackable_enemies = list(filter(lambda x: (x.champion and x.health > 0), self.enemy_team()))
        if(not self.target and len(attackable_enemies) > 0): field.find_target(self)
...
def run(champion, player_1, player_2, round_damage=0):
    ...
    while True:
        ...
        if(MILLIS() > 200000):
            ...
            break
        if(MILLIS() > 0 and MILLIS() % origin_class_stats.length['elderwood'] == 0): origin_class.elderwood(blue, red) #elderwood -trait
        if(MILLIS() > 0 and MILLIS() % origin_class_stats.threshold['hunter'][origin_class.get_origin_class_tier('blue', 'hunter')] == 0): origin_class.hunter(blue) #hunter -trait
        if(MILLIS() > 0 and MILLIS() % origin_class_stats.threshold['hunter'][origin_class.get_origin_class_tier('red', 'hunter')] == 0): origin_class.hunter(red) #hunter -trait
        for b in blue:
            field.action(b)
...
def shield(champion, action, length, function, stat, value, data):
    ...
    if(data['increase']):
        ...
        if('expires' in data and data['expires']):
            champion.add_que('shield', data['expires'], None, None, None, {'increase': False, 'identifier': value['identifier']})
    else:
        for s in champion.shields:
    ...
    if(stat == 'AS' and champion.name == 'jhin'):
        ...
        if(not ('quicksilver' in champion.items and MILLIS() <= item_stats.item_change_length['quicksilver'] and value == True and (stat == 'stunned' or stat == 'disarmed' or stat == 'blinded'))):
            if(not ('rapid_firecannon' in champion.items and value == True and stat == 'blinded')):
                if(not (champion.name == 'galio' and (MILLIS() - origin_class.galio_spawn_time[champion.team] <= origin_class_stats.cc_immune['cultist']) and (stat == 'stunned' or stat == 'disarmed' or stat == 'blinded'))):
                    end_value = value
