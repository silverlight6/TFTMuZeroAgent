...
def test_scalar_encoder_correctness(encoder, key):
    ...
    """
    initial_values = jax.random.uniform(key, shape=(100,), minval=encoder.min_value, maxval=encoder.max_value,
                                        dtype=jnp.float16)
    logits = encoder.encode(initial_values)
    values = encoder.decode(logits)
    checkify.check(jnp.allclose(initial_values, values, atol=0.1), "Scalar Encoder does not work properly! "
                                                                   "Diff (initial, decoded):\n {diff}",
                   diff=jnp.stack((initial_values, values), axis=1))
...
def test_scalar_encoder_performance(encoder, key):
    ...
    """
    initial_values = jax.random.uniform(key, shape=(1000,), minval=encoder.min_value, maxval=encoder.max_value,
                                        dtype=jnp.float16)
