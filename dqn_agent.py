...
class DQN_Agent:
    ...
    def train(self, batch):
        ...
        if error:
            ...
            state_batch = np.reshape(state_batch, (state_batch.shape[0], 1, 1094))
            next_state_batch = np.array(tmp_next_state_batch)
            next_state_batch = np.reshape(next_state_batch, (next_state_batch.shape[0], 1, 1094))
        ...
        for idx in range(0, len(target_q[1])):
            for field in range(0, 5):
                if done_batch[idx]:
                    print("In the done section")
                    target_q[field][idx][0][action_batch[idx][field]] = reward_batch[idx]
                else:
                    # print("idx = " + str(idx) + ", and field = " + str(field) +
                    #       ", with action_batch = " + str(action_batch[idx][field])
                    #       + ", and max_next_q = " + str(max_next_q[idx][field]))
