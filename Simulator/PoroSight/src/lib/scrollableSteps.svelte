<script lang="ts">
	...
	export let showPlayerSlider = true;
	...
	const calcActionsPerRound = (timeSteps: TimeStep[]) => {
		if (!timeSteps || timeSteps.length < 2) {
			return [0, battleWidth, battleWidth];
		}
		let actions = 0;
		while (actions + 1 < timeSteps.length && !timeSteps[actions + 1].battle) {
			actions += 1;
		}
	...
	function findPrevBattle(index: number) {
		...
		while (prevBattle > 0 && !timeSteps[prevBattle].battle) {
			prevBattle -= 1;
		}
	...
	function findNextBattle(index: number) {
		...
		while (nextBattle < timeSteps.length - 1 && !timeSteps[nextBattle].battle) {
			nextBattle += 1;
		}
	...
	function battleOutcome(battle) {
		if (battle.health < 0) {
			return 'death';
		} else if (battle.result === 'win' || battle.result === 'setup') {
			return 'win';
		} else {
			return 'lose';
		}
...
<div class="scrollable-container">
	<div class="flex justify-center gap-5 mb-3 align-middle">
		<div class="text-center align-middle text-lg stat-value mb-3">
			Round {computeRoundString(timeSteps[currentIndex]?.round ?? 0)}
		</div>
		{#if showPlayerSlider && playerIds.length > 1}
			<div>
				<div class="w-full flex justify-between text-xs px-2">
					{#each playerIds as _, i}
						<span>{i + 1}</span>
					{/each}
				</div>
				<input
					bind:value={currentPlayerID}
					type="range"
					min="0"
