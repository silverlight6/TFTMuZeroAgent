...
export class Game {
    ...
    getKind(): GameKind {
        return this.uiState.kind;
    }

    getPlayers(): string[] {
        const fromSummaries = this.uiState.summaries.map(summary => summary.player);
        if (fromSummaries.length) {
            const extra = Object.keys(this.uiState.players).filter((id) => !fromSummaries.includes(id));
            return fromSummaries.concat(extra);
        }
    ...
    getPlayerSummary(playerID: string): Summary {
        const found = this.uiState.summaries.find(summary => summary.player === playerID);
        if (found) {
            return found;
        }
        const state = this.uiState.players[playerID]?.states[0];
        return {
            player: playerID,
            placement: 0,
    ...
    apply(state: PlayerState, action: Action): PlayerState {
        (Object.keys(state) as (keyof PlayerState)[]).forEach((key) => {
    ...
    diff(state: PlayerState, newState: PlayerState, action: Action): PlayerDiff {
        ...
        const keyExists = (key: keyof Action) => {
