...
class TFT_Vector_Pos_Simulator:
    ...
    def __init__(self, data_generator=None, num_envs=1, preset_battle=False,
                 step_until_units_placed=False, single_player=False,
        ...
        self.envs = [TFT_Position_Simulator(
            data_generator, 0,
            preset_battle=preset_battle,
            step_until_units_placed=step_until_units_placed,
        ...
        while len(self.envs) < num_envs:
            self.envs.append(TFT_Position_Simulator(
                data_generator, index,
                preset_battle=preset_battle,
                step_until_units_placed=step_until_units_placed,
                single_player=single_player,
                render_mode=render_mode,
    ...
    def make_env(self, index):
        return TFT_Position_Simulator(
            self.data_generator, index,
            preset_battle=self.preset_battle,
            step_until_units_placed=self.step_until_units_placed,
    ...
    def vector_reset_step(self, actions: List, seeds: Optional[List[int]] = None,
...
class TFT_Single_Player_Vector_Simulator:
    ...
    def __init__(self, tft_config: TFTConfig, num_envs: int):
        ...
        Args:
            ...
            num_envs: Total number of sub environments in this VectorEnv.
        """
        self.envs = [TFT_Single_Player_Simulator(tft_config, index=0)]
        self.num_envs = num_envs
        self.observation_space = self.envs[0].observation_space
        self.action_space = self.envs[0].action_space
        ...
        # Fill up missing envs (so we have exactly num_envs sub-envs in this
        while len(self.envs) < num_envs:
            self.envs.append(TFT_Single_Player_Simulator(tft_config, index=len(self.envs)))
    ...
    def make_env(self, index=None):
        return TFT_Single_Player_Simulator(self.tft_config, index=index)
