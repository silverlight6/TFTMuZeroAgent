...
class A3C_Agent:
    def __init__(self, input_shape, t_board=None, batch_size=120, sequence_length=16):
        self.optimizer = tf.optimizers.RMSprop(learning_rate=1e-3, decay=0.99, epsilon=1e-5)
        self.action_dim = [12, 10, 9, 7, 4]
        self.mlp_block_num = 2
        self.popArtLayer = PopartLayer(1, hidden_dim=sum(self.action_dim) + 1 + 512)
    ...
    def build_a3c_model(self, input_array):
        ...
        x = tf.keras.layers.LSTM(512, activation='tanh', name='lstm_layer')(x)
        ...
        shop_output = tf.keras.layers.Dense(self.action_dim[0], activation='softmax', name='shop_layer')(x)
        item_output = tf.keras.layers.Dense(self.action_dim[1], activation='softmax', name='item_layer')(x)
        bench_output = tf.keras.layers.Dense(self.action_dim[2], activation='softmax', name='bench_layer')(x)
        board_output_x = tf.keras.layers.Dense(self.action_dim[3], activation='softmax', name='board_x_layer')(x)
        board_output_y = tf.keras.layers.Dense(self.action_dim[4], activation='softmax', name='board_y_layer')(x)
    ...
    def policy(self, state, prev_action, prev_reward, episode):
        one_hot_last_action = tf.Variable(tf.one_hot(prev_action[0], self.action_dim[0]))
        for i in range(1, len(self.action_dim)):
            one_hot_last_action = tf.concat([one_hot_last_action,
                                             tf.one_hot(prev_action[i], self.action_dim[i])], axis=0)
        one_hot_last_action = tf.concat([one_hot_last_action, [prev_reward]], axis=0)

        state_input = tf.convert_to_tensor(state[None, :], dtype=tf.float32)
        action_q, [value, _] = self.a3c_net([state_input, tf.expand_dims(one_hot_last_action, axis=0)])

        action = [int(tf.random.categorical(action_q[0], 1).numpy()),
                  int(tf.random.categorical(action_q[1], 1).numpy()),
                  int(tf.random.categorical(action_q[2], 1).numpy()),
                  int(tf.random.categorical(action_q[3], 1).numpy()),
                  int(tf.random.categorical(action_q[4], 1).numpy())]
        return action_q, action, value
    ...
    def train_step(self, batch, episode=0):
        ...
        state_batch, logit_batch, action_batch, reward_batch, prev_action_batch, prev_reward_batch = batch
        ...
        # Since the model normally gets the raw reward value anyways
        # sample_batch = self.process_rewards(vr_r, episode)
        act_re_input = tf.Variable(tf.one_hot(prev_action_batch[:, 0], self.action_dim[0]))
        for i in range(1, len(self.action_dim)):
            act_re_input = tf.concat([act_re_input, tf.one_hot(prev_action_batch[:, i], self.action_dim[i])], axis=-1)
        prev_reward_batch = tf.convert_to_tensor(self.transpose(prev_reward_batch), dtype=tf.float32)
        act_re_input = tf.concat([act_re_input, tf.expand_dims(prev_reward_batch, axis=-1)], axis=-1)
        with tf.GradientTape() as tape:
            ...
            # Moving over the sequence length because I need a output at each time step
    ...
    def actor_loss(self, probs, actions, advantage, episode=0):
    ...
    def maskInput(self, policy, logits, actions):
        ...
        #         policy_arr[i].append(policy[i][j].numpy())
        for b in range(self.batch_size):
            for s in range(self.sequence_length):
                if actions[b, 0, s] < 8 or actions[b, 0, s] > 8:
                    ...
                    # policy_arr[1][s][b] = one_hot_action.numpy()
                ...
                if actions[b, 0, s] < 9 or actions[b, 0, s] == 11:
                    ...
                    # policy_arr[2][s][b] = one_hot_action.numpy()
                ...
                if actions[b, 0, s] < 8 or actions[b, 0, s] == 9:
                    ...
                    # policy_arr[3][s][b] = one_hot_action.numpy()
                    logits[3][b][s] = tf.expand_dims(one_hot_action, axis=0)
                    one_hot_action = tf.one_hot(actions[b, 0, s], self.action_dim[4])
                    # policy_arr[4][s][b] = one_hot_action.numpy()
                    logits[4][b][s] = tf.expand_dims(one_hot_action, axis=0)
        # policy = [tf.stack(policy_arr[0]), tf.stack(policy_arr[1]), tf.stack(policy_arr[2]),
        #           tf.stack(policy_arr[3]), tf.stack(policy_arr[4])]
...
class PopartLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        # tf.clip_by_value(head_output, -1, 1)
        un_norm_output = self.std * head_output + self.mean
        # tf.clip_by_value(un_norm_output, -1, 1)
...
class Mlp(tf.Module):
    def __init__(self, hidden_size=512, mlp_dim=1024, dropout_rate=0.0):
