...
def chosen(champion, value):
    if(value): 
        ...
        if(stat_change['stat'] == 'maxmana'): items.change_stat(champion, stat_change['stat'], getattr(champion, stat_change['stat']) * stat_change['value'], 'chosen')
        else:
            items.change_stat(champion, stat_change['stat'], getattr(champion, stat_change['stat']) + stat_change['value'], 'chosen')
        return value
    return False
...
def is_trait(champion, trait):
    champion_data = origin_class_stats.origin_class
    if(trait in champion_data[champion.name]): return True
...
def divine(champion, target, attack):
    ...
    if(is_trait(champion, 'divine') and get_origin_class_tier(champion.team, 'divine') > 0 and not champion in divine_list):
        if(attack):
            ...
            for c in teams[t]:
                if(is_trait(c, 'exile')):
                    ...
                    if(applicable):
                        ...
                        c.add_que('shield', -1, None, None, {'amount': shield_size, 'identifier': shield_identifier, 'applier': c, 'original_amount': shield_size}, {'increase': True})
...
def the_boss_helper(champion, data):
    ...
    if(len(champion.own_team()) > 1):
        ...
        items.change_stat(champion, 'movement_delay', champion.movement_delay * origin_class_stats.movement_delay['the_boss'])
    ...
    if(len(champion.own_team()) == 1 or champion.pumped_up):
        ...
        for i, h in enumerate(hexes):
            d = field.distance({'y': corner[champion.team][0], 'x': corner[champion.team][1]}, {'y': h[0], 'x': h[1]}, False)
            hexes[i].append(d)
        ...
        if(tier > 0):
            for c in teams[t]:
                if(is_trait(c, 'warlord')):
                    ...
                    SP_add = origin_class_stats.SP['warlord'][tier] * (1 + wins * origin_class_stats.increasement['warlord'])
    ...
    for t in ['blue', 'red']:
        ...
        for c in teams[t]:
            if(is_trait(c, 'assassin')):
                ...
                if(tier > 0):
                    items.change_stat(c, 'crit_chance', c.crit_chance + origin_class_stats.crit_chance['assassin'][tier], 'assassin')
                    items.change_stat(c, 'crit_damage', c.crit_damage + origin_class_stats.crit_damage['assassin'][tier], 'assassin')
                ...
                if(is_trait(c, 'keeper')):
                    ...
                    for h in hexes:
                        ...
                        if(u and u.team == c.team and u.champion):
                            ...
                            u.add_que('shield', -1, None, None, {'amount': shield, 'identifier': shield_identifier, 'applier': c, 'original_amount': shield}, {'increase': True, 'expires': length})
