...
def kalista(champion, target):
    if(target):
        ...
        if(target.MR >= 0): spear_damage = spear_damage * (100/(100+target.MR)) * champion.SP
        else: spear_damage = spear_damage * (2- 100/(100 - target.MR)) * champion.SP
...
def vayne(champion, target):
    ...
    if(vayne_targets[index][2] == stats.ACTIVATE_EVERY_X_ATTACKS[champion.name]):
        ...
        champion.print(' active triggered')
        return {'damage': stats.ACTIVE_DMG[champion.name][champion.stars] * champion.SP, 'true_damage': True, 'crit_random': None, 'dodge_random': None}

    else: return {'damage': 0, 'true_damage': True, 'crit_random': None, 'dodge_random': None}
...
def warwick(champion, target):
    if(champion.ability_active):
        ...
        if(dodge_random < target.dodge): damage = 0
                ...
                if(o != champion):
                    ...
                    for t in origin_class_stats.origin_class[o.name]:
                        if(t in ww_traits):
                            ...
                            o.add_que('change_stat', stats.ABILITY_LENGTH[champion.name], None, 'AS', None, {'ezreal': as_gain})
                            break
    ...
    else: return {'damage': 0, 'true_damage': False, 'crit_random': None, 'dodge_random': None}
...
def zed(champion, target):
    ...
    if(zed_counter[index][1] == stats.ACTIVATE_EVERY_X_ATTACKS[champion.name]):
        champion.print(' active triggered')
