...
def aatrox(champion):
    ...
    if(enemy_amount < enemies_pulled + 1): enemies_pulled = enemy_amount - 1
...
def ahri(champion):
    ...
    champion.add_que('execute_function', stats.ABILITY_LENGTH[champion.name], [ahri_ability, {'y': champion.target.y, 'x': champion.target.x}])
...
def annie(champion):
    ...
    if(champion.y % 2 == 0):
        ...
        cone_center = [champion.y + cone_center_table[direction_y][direction_x][0], champion.x + cone_center_table[direction_y][direction_x][1]]
        leave_out = [champion.y + leave_out_table[direction_y][direction_x][0], champion.x + leave_out_table[direction_y][direction_x][1]]
    else:
        direction_x = spell_target[1]-champion.x + 1
        cone_center_table = [
            [[-2,-1], [-2,1], []],
    ...
    for n in neighbors:
        if(n != leave_out and field.coordinates[n[0]][n[1]] and field.coordinates[n[0]][n[1]].team != champion.team and field.coordinates[n[0]][n[1]].champion): 
    ...
    champion.add_que('shield', 0, None, None, {'amount': shield_amount, 'identifier': champion_functions.MILLIS() * shield_amount, 'applier': champion, 'original_amount': shield_amount}, {'increase': True, 'expires': stats.SHIELD_LENGTH[champion.name]})
...
def ashe(champion):
    ...
    #to ensure the changes to go through before the initial attack, we'll just set the change time to be CURRENT TIME - 1 MS
...
def ashe_helper(champion, data):
    ...
    for i in range(0, stats.ABILITY_SLICES[champion.name]):
        if(target):
            champion.attack(data['bonus_dmg'], target,  False, ' ability', champion.AD * stats.ABILITY_DAMAGE_MULTIPLIER[champion.name])


def azir(champion):
    default_ability_calls(champion)
    ability_rectangle_width = 3
    affected_hexes = field.rectangle_from_champion_to_wall_behind_target(champion, ability_rectangle_width, champion.target.y, champion.target.x)
                    ...
                    if(affected_hexes[j][i][2] <= 3):
                        ...
                        while(not push_coordinates):
                            if(i + push_counter < len(affected_hexes[j])):
                                if(field.coordinates[affected_hexes[j][i + push_counter][0]][affected_hexes[j][i + push_counter][1]] is None):
                                    push_coordinates = [affected_hexes[j][i + push_counter][0], affected_hexes[j][i + push_counter][1]]
                                else: push_counter += 1
                            else: break
                        if(push_coordinates):
...
def cassiopeia(champion):
    ...
    #if the target --> tile -line hexes are all in the champion --> tile -line, choose that end tile to be the cone end point
    line_end_point = []
    ...
    for f in four_tiles_away:
        ...
        d_from_end_point = field.distance({'y': line_end_point[0], 'x': line_end_point[1]}, {'y': f[0], 'x': f[1]}, False)
    ...
    side_points = []
    for f in four_tiles_away:
        d_from_first_corner = field.distance({'y': cone_corners[0][0], 'x': cone_corners[0][1]}, {'y': f[0], 'x': f[1]}, False)
        d_from_second_corner = field.distance({'y': cone_corners[1][0], 'x': cone_corners[1][1]}, {'y': f[0], 'x': f[1]}, False)
        d_from_end_point = field.distance({'y': line_end_point[0], 'x': line_end_point[1]}, {'y': f[0], 'x': f[1]}, False)
    ...
    for i in range(-5, 12):
        for j in range(-5,11):
            ...
            if(d_from_end_point == 2 and d_from_first_corner == 2 and d_from_second_corner == 2 and d_from_champion == 2):
                mid_hex = f
            if(d_from_end_point == 2 and d_from_first_corner == 3 and d_from_second_corner == 3 and d_from_champion == 2):
                mid_hex_secondary = f

    #if the hexes align the right way, there's two extra coords that need to be added. one next to each corner
    additional_coords = []
    if(not mid_hex_secondary and not mid_hex): mid_hex = [target.y, target.x]
    elif(not mid_hex): 
    ...
    for c in cone:
        ...
        if(c[0] >= 0 and c[0] <= 7 and c[1] >= 0 and c[1] <= 6):
            ...
            if(h and h.team != champion.team and h.champion and h not in already_targeted):
                ...
                if(not ('quicksilver' in h.items and champion_functions.MILLIS() <= item_stats.item_change_length['quicksilver'])):
                    h.print(' {} {} --> {}'.format('stunned', h.stunned, True))
                    h.stunned = True
                    h.clear_que_stunned_removal()

                    h.add_que('change_stat', stats.ABILITY_STUN_DURATION[champion.name][champion.stars], None, 'stunned', False)
                else: h.print(' not stunned because wears quicksilver')

                dmg_increase = stats.ABILITY_TARGET_INCREASE_DAMAGE_RECEIVING[champion.name]
                h.print(' {} {} --> {}'.format('receive_increased_damage', h.receive_increased_damage, round(dmg_increase * champion.SP,3)))
                h.receive_increased_damage = dmg_increase * champion.SP

                h.add_que('change_stat', stats.ABILITY_STUN_DURATION[champion.name][champion.stars], None, 'receive_increased_damage', 1)
...
def diana(champion):
    ...
    shield_amount = stats.SHIELD_AMOUNT[champion.name][champion.stars] * champion.SP
    champion.add_que('shield', 0, None, None, {'amount': shield_amount, 'identifier': shield_identifier, 'applier': champion, 'original_amount': shield_amount}, {'increase': True, 'expires': stats.SHIELD_LENGTH[champion.name]})
    ...
    for i in range(0, stats.ABILITY_TARGETS[champion.name][champion.stars]):
        ...
        orbs.append({'y': neighbors[i % n_a][0], 'x': neighbors[i % n_a][1], 'index': i%n_a, 'orbs': orbs, 'identifier': i, 'shield_identifier': shield_identifier})
...
def diana_ability(champion, data):
    ...
    if(c and c.team != champion.team and c.champion):
        ...
        if(len(data['orbs']) == 0):
            ...
            for s in champion.shields:
                if(s['identifier'] == data['shield_identifier']):
                    ...
                    champion.print(' {} {} --> {}'.format('shield', ceil(shield_before), ceil(champion.shield_amount())))
                    break
        ...
        neighbors = field.find_neighbors(champion.y, champion.x)
        data['index'] += 1
        if(data['index'] >= len(neighbors)): data['index'] = 0
...
def elise(champion):
    ...
    champion.add_que('change_stat', -1, None, 'heal_per_attack', champion.heal_per_attack + stats.ABILITY_HEALTH_PER_ATTACK[champion.name][champion.stars])
    champion.add_que('change_stat', -1, None, 'max_health', champion.max_health * stats.ABILITY_HEALTH_GAIN_PERCENTAGES[champion.name][champion.stars])
...
def evelynn(champion):
    ...
    for i in range(0, targets):
        if(i > len(enemies_around_target) - 1): break
...
def ezreal(champion):
    ...
    champion.add_que('execute_function', stats.ABILITY_LENGTH[champion.name], [ezreal_ability, {'y': champion.target.y, 'x': champion.target.x}])
...
def ezreal_ability(champion, data):
    ...
    affected_hexes = field.rectangle_from_champion_to_wall_behind_target(champion, ability_rectangle_width, target_y, target_x)

    longest_line = max([len(affected_hexes[0]),len(affected_hexes[1]),len(affected_hexes[2]), len(affected_hexes[3]), len(affected_hexes[4])])
        ...
        for j in range(0,ability_rectangle_width):
            ...
            if(len(affected_hexes[j]) > i):
                ...
                if(c and c.team != champion.team and c.champion and c not in already_targeted):
                    ...
                    c.add_que('change_stat', -1, None, 'AS', c.AS * stats.ABILITY_AS_DECREASE[champion.name][champion.stars])
                    c.add_que('change_stat', stats.ABILITY_AS_CHANGE_LENGTH[champion.name], None, 'AS', None, {'ezreal': stats.ABILITY_AS_DECREASE[champion.name][champion.stars]})
                ...
                if(c and c.team == champion.team and c.champion and c not in already_targeted and c is not champion):
                    ...
                    c.add_que('change_stat', -1, None, 'AS', c.AS * stats.ABILITY_AS_GAIN[champion.name][champion.stars])
                    c.add_que('change_stat', stats.ABILITY_AS_CHANGE_LENGTH[champion.name], None, 'AS', None, {'ezreal': stats.ABILITY_AS_GAIN[champion.name][champion.stars]})
                    already_targeted.append(c)
...
def fiora_ability(champion, data):
    ...
    if(champion.target):
        ...
        champion.target.add_que('change_stat', stats.ABILITY_STUN_DURATION[champion.name][champion.stars], None, 'stunned', False)
...
def garen(champion):
    ...
    champion.add_que('change_stat', -1, None, 'spell_damage_reduction_percentage', champion.spell_damage_reduction_percentage * stats.ABILITY_SPELL_DAMAGE_REDUCTION_PERCENTAGE[champion.name])
    champion.add_que('change_stat', stats.ABILITY_LENGTH[champion.name], None , 'spell_damage_reduction_percentage', None, {'garen': stats.ABILITY_SPELL_DAMAGE_REDUCTION_PERCENTAGE[champion.name]})


    ms = 50
    for i in range(0, stats.ABILITY_SLICES[champion.name]):
        champion.add_que('execute_function', ms + i * (stats.ABILITY_LENGTH[champion.name] / stats.ABILITY_SLICES[champion.name]), [garen_ability, {}])
        ...
        for j in range(0,ability_rectangle_width):
            ...
            if(len(affected_hexes[j]) > i):
                ...
                if(c and c.team != champion.team and c.champion and c not in already_targeted):
                    ...
                    c.add_que('change_stat', stats.ABILITY_DISARM_DURATION[champion.name][champion.stars], None, 'disarmed', False)
...
def janna(champion):
    default_ability_calls(champion)
    ally_units = []
    if(champion.team == 'blue'): ally_units = champion.blue_return()
    if(champion.team == 'red'): ally_units = champion.red_return()
    ...
    if(len(ally_units) > shielded_units): ally_units = ally_units[:shielded_units]
...
def katarina(champion):
    ...
    for i in range(0, stats.ABILITY_SLICES[champion.name]):
        champion.add_que('execute_function', i * (stats.ABILITY_LENGTH[champion.name] / stats.ABILITY_SLICES[champion.name]), [katarina_ability, {}])
...
def kayn(champion, data = {'redash': False}):
    if(not champion.stunned):
        if(not champion.target): 
            field.find_target(champion)
            champion.print(' has a new target: ' + '{:<8}'.format(champion.target.team) + '{:<8}'.format(champion.target.name))
        ...
        for e in enemies_in_range:
            ...
            if('shadow_assassin' in champion.items and champion_functions.MILLIS() < stats.ABILITY_EXTRA_DAMAGE_LENGTH[champion.name]):
            ...
            if('rhaast' in champion.items):
                damage = 0
                if(e.MR >= 0): damage = ability_dmg * (100/(100+e.MR)) * champion.SP
                else: damage = ability_dmg * (2- 100/(100 - e.MR)) * champion.SP
                champion.add_que('heal', -1, None, None, damage * stats.ABILITY_HEALTH_PER_CAST_DAMAGE_PERCENTAGES[champion.name][champion.stars])
...
def kennen(champion):
    ...
    if(champion.target and field.distance(champion, champion.target, True) <= stats.ABILITY_RADIUS[champion.name]):
        ...
        for i in range(0, stats.ABILITY_SLICES[champion.name]):
            champion.add_que('execute_function', i * (stats.ABILITY_LENGTH[champion.name] / stats.ABILITY_SLICES[champion.name]), [kennen_ability, {}])
            ...
            if(not kick_out):
                ...
                if(end_point[:2] != end_point_original):
                    ...
                    while(not kick_coords):
                        ...
                        if(coords[line[0][0]][line[0][1]] or not (line[0][0] == 0 or line[0][0] == 7 or line[0][1] == 0 or line[0][1] == 6)):
                            line = line[1:]
                        else: 
                            kick_coords = line[0]
                ...
                #find the closest hex to the corner that's free
                    ...
                    for n in hexes_in_distance:
                        ...
                        if((n[0] == 0 or n[0] == 7 or n[1] == 0 or n[1] == 6) and not coords[n[0]][n[1]]):
                            ...
                            d = field.distance({'y': n[0], 'x': n[1]}, {'y': end_point_original[0], 'x': end_point_original[1]}, False)
                ...
                if(deal or not kick_out):
                    ...
                    if(champion.target.y != kick_coords[0] and champion.target.x != kick_coords[1]):
                        line = field.line({'y': champion.target.y, 'x': champion.target.x}, {'y': kick_coords[0], 'x': kick_coords[1]})
                        for l in line:
                            c = coords[l[0]][l[1]]
                            if(c and c.team != champion.team and c.champion and c != champion.target):
                                c.add_que('change_stat', 0, None, 'stunned', True)
                                c.clear_que_stunned_removal()
                                c.add_que('change_stat', stats.ABILITY_SECONDARY_STUN_DURATION[champion.name][champion.stars], None, 'stunned', False)
                    ...
                    if(champion.target and field.distance({'y': target_y, 'x': target_x}, {'y': champion.y, 'x': champion.x}, False) > champion.range):
...
def lillia(champion):
    ...
    #if two lillia ults by the same unit were casted while the last one's stuns are still active, remove the hp checks from the que 
    """que = champion.que_return()
     for q in que:
...
def lissandra(champion):
    ...
    side_primary_neighbors = [[primary_neighbors_count[1][0], primary_neighbors_count[1][1]], [primary_neighbors_count[2][0], primary_neighbors_count[2][1]]]
    ...
    for i, m in enumerate(middle_hex_neighbors):
        distance0 = field.distance({'y': m[0], 'x': m[1]}, {'y': side_primary_neighbors[0][0], 'x': side_primary_neighbors[0][1]}, False)
        distance1 = field.distance({'y': m[0], 'x': m[1]}, {'y': side_primary_neighbors[1][0], 'x': side_primary_neighbors[1][1]}, False)
    ...
    #now find all the hexes within two distance from the orange circle that are also within two distance of the red circle 
    ...
    cone = list(set(map(tuple, middle_hex_two_distance)).intersection(set(map(tuple, secondary_middle_hex_two_distance))))

    #now add the corners (black) of the cone by finding hexes that are of 4 distance from the champion and three distance from the red circle
...
def lux(champion):
    ...
    if(len(champion.enemy_team()) > 0):
        ...
        target = None
        if(len(distances) == 0): target = [champion.target.y, champion.target.x]
        else: target = distances[0][0]
...
def morgana_ability(champion, data):
    ...
    targets = field.enemies_in_distance(champion, data['coordinates'][0], data['coordinates'][1], stats.ABILITY_RADIUS[champion.name])
    ...
    for t in targets:
        ...
        damage = 0
        if(t.MR >= 0): damage = ability_dmg * (100/(100+t.MR)) * champion.SP
        else: damage = ability_dmg * (2- 100/(100 - t.MR)) * champion.SP
    ...
    if(data['ms'] == stats.ABILITY_LENGTH[champion.name] - (stats.ABILITY_LENGTH[champion.name] / stats.ABILITY_SLICES[champion.name])):
...
def nidalee(champion):
    ...
    path = (field.line({'y': champion.y, 'x': champion.x}, {'y': enemies_distance[0].y, 'x': enemies_distance[0].x}))[1:]
...
def nunu(champion):
    ...
    if(champion.health > champion.target.health):
        true_dmg = stats.ABILITY_DMG[champion.name][champion.stars] * (1 + stats.ABILITY_DAMAGE_ADDITION_PERCENTAGE[champion.name])
...
def pyke(champion):
    ...
    if(not dash_target):
        ...
        distance_to_dash_target = field.distance({'y': champion.y, 'x': champion.x}, {'y': dash_target[0], 'x': dash_target[1]}, False)
        ...
        #find all the hexes that have three neighbors which are in the path (finding the x-marked hexes in 'pyke_ult.png')
        all_hexes = field.hexes_in_distance(0, 0, 20)
        for a in all_hexes:
            a_neighbors = field.find_neighbors(a[0], a[1])
            count = 0
            for n in a_neighbors:
                if(n in path): count += 1

            if(count == 3): extra_path.append(a)
...
def riven(champion):
    ...
    if(riven_helper(champion, {})):
        ...
        global riven_mage_counter
        default_ability_calls(champion)
        #riven_counter needs to sustain multiple rivens on the field, so there's gonna be every riven's data on the same array
        found = False
        index = -1
        ...
        if(len(champion.enemy_team()) > 0):
            ...
            #damage ALL neighboring enemies
                ...
                if(shield_identifier in riven_identifier_list):
                    champion.add_que('shield', 0, None, None, {'amount': shield_amount + 0.001, 'identifier': shield_identifier, 'applier': champion, 'original_amount': shield_amount + 0.001}, {'increase': True})

                riven_identifier_list.append(shield_identifier)

                champion.add_que('shield', 0, None, None, {'amount': shield_amount, 'identifier': shield_identifier, 'applier': champion, 'original_amount': shield_amount}, {'increase': True})
    ...
    else: champion.attack()

#check if she has old shields left
def riven_helper(champion, data):
    if(len(champion.shields) == 0): return True
    own_shields = list(filter(lambda x: (x['applier'] == champion and x['original_amount'] == stats.SHIELD_AMOUNT[champion.name][champion.stars] * champion.SP), champion.shields))
    if(len(own_shields) == 0): return True
    else: return False
...
def sejuani(champion):
    ...
    champion.add_que('execute_function', stats.ABILITY_LENGTH[champion.name], [sejuani_ability, {'target': [target.y, target.x]}])
...
def sett(champion):
    ...
    if(len(champion.enemy_team()) > 0):
        ...
        two_from_champion = field.hexes_distance_away(champion.y, champion.x, 2, True)
        two_from_n0 = field.hexes_distance_away(one_distance_neighbors[0][0][0], one_distance_neighbors[0][0][1], 2, True)
        two_from_n1 = field.hexes_distance_away(one_distance_neighbors[1][0][0], one_distance_neighbors[1][0][1], 2, True)
...
def shen(champion):
    ...
    shield_identifier = champion_functions.MILLIS() * stats.SHIELD_AMOUNT[champion.name][champion.stars] * champion.SP
    shield_amount = stats.SHIELD_AMOUNT[champion.name][champion.stars] * champion.SP
    champion.add_que('shield', 0, None, None, {'amount': shield_amount, 'identifier': shield_identifier, 'applier': champion, 'original_amount': shield_amount}, {'increase': True, 'expires': stats.ABILITY_LENGTH[champion.name][champion.stars]})
...
def sylas(champion):
    ...
    distance = field.distance({'y': champion.y, 'x': champion.x}, {'y': champion.target.y, 'x': champion.target.x}, False)
    smash_line = []

    first_hex = field.line({'y': champion.y, 'x': champion.x}, {'y': champion.target.y, 'x': champion.target.x})[1]
...
def talon(champion):
    ...
    if(not champion.target or (champion.target != original_target)):
        ...
        if(len(enemies) > 0):
            ...
            champion.add_que('execute_function', stats.ABILITY_LENGTH[champion.name], [talon_ability, {'target': next_target}])
    ...
    if(data['target'].health > 0):
        ...
        if(jump_target):
            ...
            for e in champion.enemy_team():
                if(e.target == champion): field.find_target(e)
...
def teemo(champion):
    ...
    if(not ((origin_class.get_origin_class_tier(champion.team, 'mage') > 0 and origin_class.is_trait(champion, 'mage')))):
        for i in range(0, 5):
            que = champion.que_return()
            for q in que:
                for t in targets:
                    if(q[1] is champion and q[0] == 'execute_function' and q[3][0] == teemo_ability and 'target' in q[3][1] and  q[3][1]['target'] == t): 
                        que.remove(q)
            champion.que_replace(que)
...
def twistedfate_ability(champion, data):
    ...
    if(y_diff > 0):
        for i in range(champion.y, 8):
            if(x_diff ==  1 and i % 2 == 1 and i != champion.y): x += x_diff
            if(x_diff == -1 and i % 2 == 0 and i != champion.y): x += x_diff
            line.append([i, x])
...
def vi(champion):
    ...
    for a in affected_hexes:
        ...
        if(c and c.team != champion.team and c.champion):
            ...
            #to make sure there are no double reductions, check how long ago the target's armor was reduced last time by this vi
            #the armor list elements are of syntax: [reducer, target, millseconds_when_last_reduced]
            #search for target entries
            armor_history = (list(filter(lambda x: x[1] == c, vi_armor_list)))
            can_be_changed = False
            for a in armor_history:
                #there can be multiple vis, so make sure that this entry was done by the currenet vi
                if(a[0] == champion):

                    #amount of ms when reduced last time
                    diff = champion_functions.MILLIS() - a[2]
            ...
            if(can_be_changed):
                ...
                c.add_que('change_stat', stats.ABILITY_LENGTH[champion.name], None, 'armor', None, {'vi': stats.ABILITY_ARMOR_DECREASE[champion.name][champion.stars]})
...
def warwick(champion):
    default_ability_calls(champion)
    champion.add_que('change_stat', -1, None, 'ability_active', True)
    champion.add_que('change_stat', -1, None, 'lifesteal', champion.lifesteal + stats.LIFESTEAL[champion.name][champion.stars])
...
def xinzhao(champion):
    ...
    champion.add_que('change_stat', -1, None, 'armor', champion.armor + stats.ABILITY_ARMOR_MR_INCREASE[champion.name][champion.stars])
    champion.add_que('change_stat', -1, None, 'MR', champion.MR + stats.ABILITY_ARMOR_MR_INCREASE[champion.name][champion.stars])
...
def yasuo(champion):
    ...
    else: champion.attack(bonus_dmg, champion.target)
    apply_attack_cooldown(champion)
...
def yone(champion):
    ...
    if(champion.maxmana == stats.MAXMANA[champion.name]):
        if(not champion.target): field.find_target(champion)
        path = field.rectangle_from_champion_to_wall_behind_target(champion, stats.ABILITY_RADIUS[champion.name], champion.target.y, champion.target.x)
        ...
        if(enemies > 0):
            ...
            for pp in path:
                for p in pp:
                    ...
                    if(c and c.team != champion.team and c.champion and c not in already_targeted):
                        ...
                        c.print(' {} {} --> {}'.format('MR', c.MR, c.MR * stats.ABILITY_ARMOR_MR_DECREASE[champion.name]))
                        c.print(' {} {} --> {}'.format('armor', c.armor, c.armor * stats.ABILITY_ARMOR_MR_DECREASE[champion.name]))
            ...
            if(yone_helper(champion) > 0):
                champion.print(' {} {} --> {}'.format('maxmana', champion.maxmana, stats.SECONDARY_MAXMANA[champion.name]))
...
def yuumi(champion):
    ...
    heal_amount = (first_target.max_health - first_target.health) * stats.ABILITY_HEALTH_GAIN_PERCENTAGES[champion.name][champion.stars] * champion.SP
    first_target.add_que('heal', -1, None, None, heal_amount)
    as_gain = (stats.ABILITY_AS_GAIN[champion.name][champion.stars] - 1) * champion.SP + 1

    first_target.add_que('change_stat', -1, None, 'AS', first_target.AS * as_gain)
    first_target.add_que('change_stat', stats.ABILITY_AS_CHANGE_LENGTH[champion.name], None, 'AS', None, {'ezreal': as_gain})
    ...
    if(second_target):
        ...
        #fixing a bug which comes from stacking stat changes in the same milliseconds. yuumi will instantly heal and buff the second ally if she is a mage

        heal_amount = (second_target.max_health - second_target.health) * stats.ABILITY_HEALTH_GAIN_PERCENTAGES[champion.name][champion.stars]  * champion.SP
        ...
        second_target.add_que('change_stat', stats.ABILITY_AS_CHANGE_LENGTH[champion.name] + pause, None, 'AS', None, {'ezreal': as_gain})
...
def galio(champion):
    ...
    if(champion.stars >= 2):
        new_damage_receiving = champion.receive_decreased_damage * stats.ABILITY_TARGET_DECREASE_DAMAGE_RECEIVING[champion.name]
