...
class Trainer(object):
    ...
    def train_network(self, batch, train_step):
        ...
        observation, action_history, value_mask, policy_mask, target_value, target_policy, importance_weights = batch

        self.adjust_lr(train_step)

        predictions = self.compute_forward(observation, action_history)

        self.compute_loss(predictions, target_value, target_policy,  value_mask, policy_mask, importance_weights)
    ...
    def compute_loss(self, predictions, target_value, target_policy, value_mask, policy_mask, importance_weights):
        ...
        self.loss = torch.sum(
            value_loss * config.VALUE_LOSS_SCALING + policy_loss * config.POLICY_LOSS_SCALING, -1).to(config.DEVICE)
